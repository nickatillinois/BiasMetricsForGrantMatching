...
true_labels = df['GT_RACE'].apply(lambda x: 1 if x == 'b' else 0)
...
class_index = df_probs.columns.get_loc('black')  # Assuming 'black' is one of the columns
...
# Plot Model's Predicted Percentage of 'black'
plt.plot(thresholds, model_black_percentage, label='Model % black Predictions (%)', color='brown')


# Add a horizontal line for the ground truth percentage where label is 'black'
plt.axhline(y=ground_truth_black_percentage, color='black', linestyle='--', label='Ground Truth % (black)')

# Add titles and labels
#plt.title('Percentage of False Positives, False Negatives, True Positives, True Negatives, Accuracy, F1 Score, and Model % black Predictions vs. Threshold')
plt.title('Black')
...
model_black_percentage = [0 if np.isnan(x) else x for x in model_black_percentage]


# Calculate the number of 'black' records
num_black_records = df_probs['true_labels'].sum()
...
# Display total number of records and number of 'black' records below the plot
#plt.figtext(0.5, 0.001, f'Total number of records: {num_samples}, Number of true black records: {num_black_records}, '
...
plt.savefig('easy_graph_black.png')
# Show the plot
plt.show()
