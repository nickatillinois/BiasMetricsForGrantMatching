...
true_labels = df['GT_RACE'].apply(lambda x: 1 if x == 'w' else 0)
...
recall_percentage = []
model_white_percentage = []

# Number of samples
num_samples = len(df_probs)

# Define the class of interest
class_index = df_probs.columns.get_loc('white')  # Assuming 'white' is one of the columns

# Calculate the ground truth percentage where the label is 'white'
ground_truth_white_percentage = (df_probs['true_labels'].sum() / num_samples) * 100
...
for threshold in thresholds:
    ...
    # Calculate model percentage of 'white' predictions
    white_model_percentage = (df_probs.iloc[:, class_index] > threshold).mean() * 100
    ...
    model_white_percentage.append(white_model_percentage)
...
# Plot Model's Predicted Percentage of 'white'
plt.plot(thresholds, model_white_percentage, label='Model % white Predictions (%)', color='brown')


# Add a horizontal line for the ground truth percentage where label is 'white'
plt.axhline(y=ground_truth_white_percentage, color='black', linestyle='--', label='Ground Truth % (white)')

# Add titles and labels
#plt.title('Percentage of False Positives, False Negatives, True Positives, True Negatives, Accuracy, F1 Score, and Model % white Predictions vs. Threshold')
plt.title('White')
...
model_white_percentage = [0 if np.isnan(x) else x for x in model_white_percentage]


# Calculate the number of 'white' records
num_white_records = df_probs['true_labels'].sum()
...
# Display total number of records and number of 'white' records below the plot
#plt.figtext(0.5, 0.001, f'Total number of records: {num_samples}, Number of true white records: {num_white_records}, '
...
plt.savefig('easy_graph_white.png')
# Show the plot
plt.show()
