...
true_labels = df['GT_RACE'].apply(lambda x: 1 if x == 'a' else 0)
...
recall_percentage = []
model_asian_percentage = []

# Number of samples
num_samples = len(df_probs)

# Define the class of interest
class_index = df_probs.columns.get_loc('asian')  # Assuming 'asian' is one of the columns

# Calculate the ground truth percentage where the label is 'asian'
ground_truth_asian_percentage = (df_probs['true_labels'].sum() / num_samples) * 100
...
for threshold in thresholds:
    ...
    # Calculate model percentage of 'asian' predictions
    asian_model_percentage = (df_probs.iloc[:, class_index] > threshold).mean() * 100
    ...
    model_asian_percentage.append(asian_model_percentage)
...
# Plot Model's Predicted Percentage of 'asian'
plt.plot(thresholds, model_asian_percentage, label='Model % asian Predictions (%)', color='brown')


# Add a horizontal line for the ground truth percentage where label is 'asian'
plt.axhline(y=ground_truth_asian_percentage, color='black', linestyle='--', label='Ground Truth % (asian)')

# Add titles and labels
#plt.title('Percentage of False Positives, False Negatives, True Positives, True Negatives, Accuracy, F1 Score, and Model % asian Predictions vs. Threshold')
plt.title('Asian')
...
model_asian_percentage = [0 if np.isnan(x) else x for x in model_asian_percentage]


# Calculate the number of 'asian' records
num_asian_records = df_probs['true_labels'].sum()
...
# Display total number of records and number of 'asian' records below the plot
#plt.figtext(0.5, 0.001, f'Total number of records: {num_samples}, Number of true asian records: {num_asian_records}, '
...
plt.savefig('easy_graph_asian.png')
# Show the plot
plt.show()
