...
small_threshold = df['GT_VALUE'].quantile(0.33)
medium_threshold = df['GT_VALUE'].quantile(0.66)
...
df['GRANT_CATEGORY'] = df['GT_VALUE'].apply(categorize_grant)
...
def calculate_mean_from_string_list(string_list):
    float_list = list(map(float, string_list.split(',')))
...
def get_top_1_grant(df, model_name, dataset, version):
    ...
    topn_appval = topn_appval.apply(calculate_mean_from_string_list)
    return topn_appval
...
avg_grant_values_wi = average_grant_top1_val('p', 'wi','all', 'all')
print(avg_grant_values_wi)
avg_grant_values_w = average_grant_top1_val('p', 'w','all', 'all')
...
plt.tight_layout()
plt.savefig('combined_grant_value_plots_b_top5.png')
plt.show()
plt.close()
"""
...
plt.savefig('comparison_grant_value_plots_b_top5.eps', bbox_inches='tight')
plt.show()
